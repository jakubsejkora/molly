...
Molly is a native macOS menu bar + dashboard app (Swift / SwiftUI / AppKit) that keeps a Mac awake for long agent or CI jobs, with an optional jittered network-probe lane to trim idle hotspot disconnects. macOS 14 Sonoma+, Apple Silicon. Currently at **v0.4.0** (pre-1.0 preview).
